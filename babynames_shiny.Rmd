...
simple Shiny interface to the 
[Social Security names data](https://www.ssa.gov/oact/babynames/background.html). 
You simply type a name. Lookups are case-sensitive by default.

Note that:

* By checking the "Partial name match" box, you will get every name which contains the name you enter. Try checking the box
* In most cases, checking the "Ignore capitalization" box will matter only when you also check the "Partial name match" box. 
...
server <- function(input, output) {
   output$namePlot <- renderPlot({
     ...
     output$nametbl = renderTable(namelist)
     babynames %>% 
